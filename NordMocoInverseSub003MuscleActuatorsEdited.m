% -------------------------------------------------------------------------- %
% OpenSim Moco: NordMocoInverseMuscleActuators.m      
...
tempModel.append(ModOpIgnoreTendonCompliance());
tempModel.append(ModOpIgnorePassiveFiberForcesDGF());
tempModel.append(ModOpScaleActiveFiberForceCurveWidthDGF(1.5));
tempModel.append(ModOpScaleMaxIsometricForce(2));
...
inverse.set_initial_time(5); %start from like later times   2.995
inverse.set_final_time(8);%decrease the time 8.685
...
for i = 0:forceSet.getSize()-1
    ...
    if contains(string(forcePath), 'pelvis')
        excitegoal.setWeightForControl(forcePath,0); % set the pelvis residuals controls to 0, don't care what the pelvis residuals are
    end
    if contains(string(forcePath), 'reserve_jointset')
        excitegoal.setWeightForControl(forcePath, 10); %discourage the use of reserve actuators
    end
    if contains(string(forcePath), 'excitation')
        excitegoal.setWeightForControl(forcePath, 0.1); %encourage the use of muscles
    end
end
...
solutionFilePath = fullfile('G:', 'Shared drives', 'HPL_Drive', 'Nord Sprinting Study', subjectID, [subjectID, 'Nord', rep, '_MocoInverse_solution_point01.sto']);
